...
* type from $CondTypeCode (required) 
...
* content.format from $CondFormatCode (required)
* content.attachment.contentType from $CondContentType
//* context.event from $CondEventCode (required)
//* context.facilityType from $ApdFacilityType (required)
//* context.practiceSetting from $ApdPracticeSetting (required)
* extension.valueCoding from $CondHomeCommunityID (required)
...
* content.format = $MedComFormatOID#urn:ad:dk:medcom:cond-v1.0:full	"DK COND schema"
